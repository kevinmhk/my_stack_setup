...
- Installs bubblewrap on Linux via native Debian/RHEL package managers
...
- On Linux, the script installs bubblewrap natively with `apt-get install -y bubblewrap`, `dnf -y install bubblewrap`, or `yum -y install bubblewrap` when `bwrap` is not already available.
